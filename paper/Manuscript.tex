...
%% (No BibTeX step: the bibliography below is a manually written
%% thebibliography environment, not a .bib database, so bibtex has
%% nothing to process. Two pdflatex passes are still required for
%% \cite/\ref/\label numbers to resolve -- this is completely normal
%% and not an error; the FIRST pass will show "??" or [?] for every
%% citation and cross-reference, and the SECOND pass resolves them all.
%% On Overleaf, use Menu -> "Recompile from scratch" once if citations
%% still show as "?" after a normal recompile.)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%% Force strictly numeric, bracketed citations -- e.g. [3] or [5,7] --
%% with NO author name or year ever printed inline. elsarticle.cls
%% already \RequirePackage{natbib} internally, so re-issuing
%% \usepackage[numbers]{natbib} here causes a fatal "Option clash for
%% package natbib" (confirmed by an actual compile attempt). natbib's
%% own \setcitestyle command reconfigures the already-loaded package
%% at runtime instead of reloading it, which is safe.
...
%% discusses them. Every table/figure below additionally uses [H] (from
%% the float package above) rather than [htbp], which pins it exactly
%% where it is written in the source instead of letting LaTeX's float
%% queue push it to a later page -- the cause of a table appearing to
%% "go missing" while its caption/number still shows up (the caption is
%% part of the same float and would have been pushed along with it; if
%% you still cannot find a table, search the PDF for its caption text
%% rather than assuming it is near the paragraph that cites it).
...
reliability measurement. PestMA~\cite{shi2025pestma} takes the
multi-agent decomposition furthest within a single domain: an
Editor--Retriever--Validator workflow for pest-management advice, where
the Validator's post-hoc check raises accuracy from 86.8\% to 92.6\%.
That is direct empirical evidence that adding a validating agent
improves reliability, which motivates rather than duplicates the
governance layer proposed here: PestMA validates a single decision
after the fact, whereas TRUSTAI gates every specialist invocation
before execution and carries a persistent, evolving reliability
estimate across queries rather than a one-shot check. OpenAg~\cite{thudumu2026openag}
is the most architecturally ambitious system reviewed, combining
domain-specific foundation models, a neural agricultural knowledge
graph, adaptive multi-agent reasoning and causal explainability into a
single framework aimed at agricultural general intelligence; as of the
version reviewed it is presented as a design and framework paper, with
no reported quantitative evaluation, which places it at the opposite
end of the rigour spectrum from CottonBot within the same literature.
...
\cite{shi2025pestma} & PestMA pest-management MAS & Editor--Retriever--Validator agents & Accuracy 86.8\% $\to$ 92.6\% after validation & Single domain (pest management); post-hoc validation, not a pre-execution trust gate \\
\cite{thudumu2026openag} & OpenAg agricultural AGI framework & Foundation models + knowledge graph + multi-agent reasoning & Architecture and design proposal & No reported quantitative evaluation as of the version reviewed \\
...
\subsubsection{Stage 1: crop knowledge}
...
\subsubsection{Stage 2: data acquisition}

Stage 2 invokes the five collectors of Table~\ref{tab:sources}, each
independently exception-isolated.

\subsubsection{Stage 3: context fusion}

Stage 3 merges the five collectors' outputs into a
...
and flagged \texttt{generation\_\allowbreak{}mode = deterministic\_\allowbreak{}fallback}. Stage 9
...
\begin{table}[H]
\centering
\caption{Ablation across four architectural conditions on identical fixtures, all four now run on the full $N=105$ benchmark used throughout this paper. An earlier version of this study reported the ablation on an archived ten-scenario ($N=10$) run predating the dataset expansion; this table replaces it and is internally consistent with Table~\ref{tab:overall}. Mean specialists run counts specialist agents that actually completed per scenario (out of five), averaged over the benchmark; task success is omitted here as every condition reached 100.0\% (Table~\ref{tab:overall}), which carries no discriminative information across conditions. Recommendation quality is LLM-judged and was evaluated for condition 4 only this round (see text); it is reported as not available (N/A) rather than estimated for conditions 1--3.}
\label{tab:ablation}
\adjustbox{max width=\textwidth}{%
\begin{tabular}{@{}lcccc@{}}
\toprule
\textbf{Condition} & \textbf{Mean specialists run} & \textbf{Rule-consist.} & \textbf{Rec.\ quality (1--5)} & \textbf{Mean latency (s)} \\
\midrule
1. Single agent, no fusion & 1.00 & 13.3\% & N/A & 34.8 \\
2. Multi-agent, deterministic merge & 3.07 & 80.0\% & N/A & 88.2 \\
3. Multi-agent + collaborative reasoning & 3.10 & 81.9\% & N/A & 154.9 \\
4. Full system + TRUSTAI & 3.11 & 84.8\% & 3.86 & 215.7 \\
\bottomrule
\end{tabular}
}
\end{table}
...
(\texttt{market\_\allowbreak{}increasing\_\allowbreak{}wheat}) returned a non-empty record set and
...
distributed. Table~\ref{tab:failures} lists all nine, grouped by the
three failure patterns discussed below.

\begin{table}[H]
\centering
\caption{The nine genuine (non-probe) wrong decisions at $N=105$, grouped by failure pattern. None have been fixed in the codebase evaluated here.}
\label{tab:failures}
\adjustbox{max width=\textwidth}{%
\begin{tabular}{@{}lp{6.0cm}p{3.4cm}p{3.4cm}@{}}
\toprule
\textbf{Pattern} & \textbf{Scenario} & \textbf{Ground truth} & \textbf{Actual decision} \\
\midrule
\multirow{5}{*}{Vegetation-detection defect}
 & vegetation\_critical\_suitability\_cotton & Field Inspection & Immediate Irrigation \\
 & vegetation\_critical\_suitability\_mango & Field Inspection & Continue Monitoring \\
 & vegetation\_critical\_suitability\_orange & Field Inspection & Continue Monitoring \\
 & vegetation\_critical\_suitability\_rice & Field Inspection & Continue Monitoring \\
 & vegetation\_critical\_yield\_cotton & Field Inspection & Continue Monitoring \\
\midrule
Signal-precedence defect & compound\_water\_market\_apple & Immediate Irrigation & Prepare for Harvest and Selling \\
\midrule
\multirow{3}{*}{Negative-control / tomato-nitrogen anomaly}
 & baseline\_normal\_tomato & Continue Monitoring & Apply Nitrogen Fertilizer \\
 & baseline\_normal\_mango & Continue Monitoring & Field Inspection \\
 & water\_stress\_suitability\_tomato & Immediate Irrigation & Apply Nitrogen Fertilizer \\
\bottomrule
\end{tabular}
}
\end{table}
...
\texttt{vegetation\_\allowbreak{}critical\_\allowbreak{}suitability\_\allowbreak{}cotton} again returns
...
\texttt{compound\_\allowbreak{}water\_\allowbreak{}market\_\allowbreak{}apple} perturbs both water stress
...
active decision: \texttt{baseline\_\allowbreak{}normal\_\allowbreak{}tomato} returned \emph{Apply
Nitrogen Fertilizer} despite soil nitrogen being reported \texttt{High},
and \texttt{baseline\_\allowbreak{}normal\_\allowbreak{}mango} returned \emph{Field Inspection}
with no corresponding perturbation. The remaining case,
\texttt{water\_\allowbreak{}stress\_\allowbreak{}suitability\_\allowbreak{}tomato}, returned \emph{Apply
...
\bibitem{shi2025pestma}
H.~Shi, S.~Li, Z.~Yuan, P.~Yang,
PestMA: LLM-based multi-agent system for informed pest management,
arXiv:2504.09855 (2025).

\bibitem{thudumu2026openag}
S.~Thudumu, J.~Fisher,
OpenAg: Democratizing agricultural intelligence,
arXiv:2506.04571 (2026).
